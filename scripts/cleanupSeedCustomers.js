// Run with: node scripts/cleanupSeedCustomers.js         (dry-run — shows what would be deleted)
// Run with: node scripts/cleanupSeedCustomers.js --confirm (actually deletes)
...
// Paste of the seed customer list to remove (only removed if they have NO visit dates)
const SEED_NAMES = [
...
const NORMALIZED_SEED_NAMES = new Set(SEED_NAMES.map(norm));
...
async function run() {
  ...
  const all = await Maintenance.find({}, { customerName: 1, lastVisitDate: 1, nextVisitDate: 1 });

  const toDelete = [];
  const skippedHasDates = [];

  for (const doc of all) {
    if (!NORMALIZED_SEED_NAMES.has(norm(doc.customerName))) continue;

    const hasDates = !!doc.lastVisitDate || !!doc.nextVisitDate;
    if (hasDates) {
      skippedHasDates.push(doc.customerName);
    } else {
      toDelete.push(doc);
    }
  }

  console.log(`\nMatched ${toDelete.length + skippedHasDates.length} of ${SEED_NAMES.length} names in the list.\n`);

  if (skippedHasDates.length > 0) {
    console.log(`KEEPING (has visit dates) — ${skippedHasDates.length}:`);
    skippedHasDates.forEach(n => console.log('  •', n));
    console.log('');
  }

  console.log(`${confirm ? 'DELETING' : 'WOULD DELETE (dry-run)'} — ${toDelete.length}:`);
  toDelete.forEach(d => console.log('  •', d.customerName));
